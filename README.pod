...
and parsed B<in order>. Builders may draw in an additional hookscript for use
at runtime.
...
adding files to the cpio image from builders. Leading path componenets, if not
...
as the builder to the image, to be run during bootstrap.
